...
有三个班可供选择：苏丰+汪亮；戴海鹏+汪亮；唐杰+刘杰

一般建议选择带有“汪亮”的课程，因为他负责pa部分，讲的很好；23级强基班大部分同学选择了“苏丰+汪亮”这一组合。理论课则比较昏昏欲睡，这一点从到课人数可以直观的看出。

### 25Fall情况：

有三个班可供选择：苏丰+汪亮；戴海鹏+汪亮；唐杰+刘杰。前两个班讲x86/64，后一个班讲RISCV，两边课程内容不同。

同样建议选择带有“汪亮”的课程，因为他负责的PA部分讲的很好。

**关于x86/64班级**：教材改用[《计算机系统：基于x86+Linux平台》](https://book.douban.com/subject/36790862/)，实际理论教学中x86和x64的内容有所混杂。课本作业部分x86和x64混杂，PA部分仍然是i386模拟器，LAB部分是x64，考试主要考察x86。
...
期末考试很难，大部分同学都有相当多的题目不会做，期末考试时间也延长了20分钟。原定分数比例是50%期末考试，笔者猜想应该是减少了期末分数占比的。

即使完成了全部pa内容，也只能勉强得到80左右的分数。（大班均分70多）做一些pa的附加内容可以达到90分。
...
**这门课推荐的打开方式是：** 上理论课建立基本概念 → 通过作业搞懂理论课知识 → 通过Lab/自己动手在机器上实验深入理解理论课知识 → 完成 PA
