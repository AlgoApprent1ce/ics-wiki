![image](https://github.com/user-attachments/assets/c507ab84-5490-4cc8-a747-8f0e13d38388)这门课是一门计算机学院的课，是信息与计算科学的平台课，因此不得不品尝
...
# 题目描述 给定一个简单图，判断这个图是否是可平面图。
## 输入格式 第一行输入 `n m`，`n` 是顶点数，`m` 是边数；之后输入 `m` 行 `u v`，代表 `u` 和 `v` 之间有一条边相连。 
## 输出格式 如果该图是可平面图，在第一行输出`1`；如果该图不是可平面图，在第一行输出`0`。 
## 样例数据 
