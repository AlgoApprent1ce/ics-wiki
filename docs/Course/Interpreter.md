...
注：本条注释的笔者没有 Java 基础，因此做实验相对吃力一些；单次实验耗时约 20 小时（含设计和debug）；但确实是一次很好的全面入门 Java 的机会。OJ 的保底机制相对友好，存在大量基础测试用例；即使是一个错到没边的实现可能也可以获得大量分数（笔者一个完全错误的面向对象设计在 Lab 3 获得了 80 分）。和编译原理一样：可能很容易搓出一个 80 分的编译器，但它距离 100 分的距离并不比从 0 到 80 更短。

## 给分
### 2025 Fall
40% 签到 + 60% 实验，其中实验比例同上（20:30:30:20）
根据3个样本得出结论：给分很好，Lab只要认真做95+也很容易，拿到100也不是梦（）。大部分测试数据强度不高，仅实现基础功能已经可以获得大半的分数。
